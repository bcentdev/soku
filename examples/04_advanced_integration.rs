...
use soku::core::models::{BuildConfig, ModuleInfo};
use soku::utils::{BuiltInTransformers, CustomTransformer, Plugin, PluginContext, Result};
...
impl Plugin for ProductionPlugin {
    ...
    async fn after_build(
        ...
        let total_size: usize = result.output_files.iter().map(|f| f.size).sum();
        ...
        if let Some(stats) = &result.tree_shaking_stats {
            println!(
                "   - Tree shaking: {} exports removed ({:.1}% reduction)",
                stats.removed_exports, stats.reduction_percentage
            );
        }
...
async fn main() -> Result<()> {
    ...
    entries.insert(
        "admin".to_string(),
        PathBuf::from("./demo-project/src/admin.js"),
    );
    entries.insert(
        "worker".to_string(),
        PathBuf::from("./demo-project/src/worker.js"),
    );
    ...
    let service =
        soku::core::services::SokuBuildService::new(fs_service, js_processor, css_processor)
            ...
            // Add build metadata
            .with_transformer(CustomTransformer::function("add-metadata", |code| {
                let metadata = format!(
                    "/* Build Info */\n\
                     /* Date: {} */\n\
                     /* Version: 1.0.0 */\n\
